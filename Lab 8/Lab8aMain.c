...
int main(void)
    ...
    InitializeHardware(HEADER, "Lab 8a: Fixed vs Floating Point") ;
	...
	while (1)
		...
		for (col = 0; col < COLS; col++)
			...
			floatSine = FloatSine(radians, ovhd) ;
			fixedSine = FixedSine(radians, ovhd) ;
...
static float FloatSine(float radians, unsigned ovhd)
    {
	...
	total += CountCycles(FloatPoly, iparams, &radians, results) ;
	count++ ;
	...
	if (sine < 0.0)
		...
		{
		PutStringAt(FLT_TXTROW, MSG_TXTCOL, "FloatPoly: %3u clock cycles (avg)", (total + count/2)/count - ovhd) ;
		total = count = 0 ;
		}
...
static float FixedSine(float radians, unsigned ovhd)
    {
	static unsigned total = 0 ;
	static unsigned count = 0 ;
	static unsigned error = 0 ;
	...
	total += CountCycles(FixedPoly, iparams, iparams, results) ;
	count++ ;
	...
	if (sine < 0)
		...
		{
		PutStringAt(FXD_TXTROW, MSG_TXTCOL, "FixedPoly: %3u clock cycles (avg)", (total + count/2)/count - ovhd) ;
		total = count = 0 ;
		}
