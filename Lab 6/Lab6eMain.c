...
#define	SAMPLES_PER_SEC			100		// Gyro output data rate
#define	ZERO_RATE_SAMPLES		32		// Number of gyro samples in running average
#define	SENSITIVITY				0.0875	// Converts integer samples to degrees/sec
...
static inline int _iabs(int x)
	...
	asm
		(
		"EOR	%[reg1],%[reg0],%[reg0],ASR 31	\n\t"
		"ADD	%[reg0],%[reg1],%[reg0],LSR 31	"
		: [reg1]	"=r"	(temp),
		  [reg0]	"+r"	(x)
		) ;
...
int main()
	{
	float degr_roll, degr_ptch, old_ptch, old_roll, dps[3] ;
	uint32_t curr_time,  down_timeout, tilt_timeout ;
	int row, col ;
	uint8_t sts ;
    ...
    while (GameOn)
		...
		if ((sts & GYRO_ZYXDA_FLAG) != 0)
			...
			degr_ptch += (dps[0] + old_ptch) / (2 * SAMPLES_PER_SEC) ;
			degr_roll += (dps[1] + old_roll) / (2 * SAMPLES_PER_SEC) ;
			old_ptch = dps[0] ;
			old_roll = dps[1] ;
			}
...
static void GetVelocity(float degrees_per_sec[])
	{
	static int16_t samples[ZERO_RATE_SAMPLES][3] = {0} ;
	static int zero_rate_level[3] = {0} ;
	int channel, sample, total, value ;
	static BOOL initialized = FALSE ;
	static int index = 0 ;

	GYRO_IO_Read((uint8_t *) samples[index], GYRO_DATA_REG, sizeof(samples[0])) ;
	...
	for (channel = 0; channel < 3; channel++)
		...
		if (!initialized)
			{
			total = 0 ;
			for (sample = 0; sample < ZERO_RATE_SAMPLES; sample++)
				{
				total += samples[sample][channel] ;
				}
			// Update zero-rate-level if gyro is stationary
			zero_rate_level[channel] = total / ZERO_RATE_SAMPLES ;
			}

		// subtract zero-rate-level and compute angular velocity
		value = samples[index][channel] - zero_rate_level[channel] ;
		if (_iabs(value) < 100) value = 0 ;
		degrees_per_sec[channel] = SENSITIVITY * value ;
		}

	index = (index + 1) % ZERO_RATE_SAMPLES ;
